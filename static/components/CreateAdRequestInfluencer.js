const CreateAdRequestInfluencer = {
    template: `
    ...
    methods: {
        async submitInfo(){
            ...
            const url = window.location.origin + `/api/influencer_adrequests/` + this.$store.state.id;
            const res = await fetch(url, {
                method: "POST",
                headers: {'Authentication-Token': sessionStorage.getItem('token'), "Content-Type": "application/json"},
                body: JSON.stringify({messages: this.form.messages, requirements: this.form.requirements, payment_amount: payment_amount, influencer_negotiation_amount: payment_amount, status: "pending_sponsor_approval", campaign_id: campaign_id}),
...
export default CreateAdRequestInfluencer;